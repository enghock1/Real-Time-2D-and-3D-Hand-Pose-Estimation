...
def draw_3d_skeleton(pose_cam_xyz, image_size, elev_angle=-85, azim_angle=-75):
    """
    ...
    mean = pose_cam_xyz.mean(axis=0)
    std = pose_cam_xyz.std(axis=0)
    pose_cam_xyz = (pose_cam_xyz-mean)/std
...
def save_batch_image_with_mesh_joints(batch_images, est_pose_uv,
    ...
    for id_image in range(num_images):
        ...
        skeleton_3d_cam_view = draw_3d_skeleton(pose_xyz, image.shape[:2])
        skeleton_3d_another_view = draw_3d_skeleton(pose_xyz, image.shape[:2], elev_angle=-150, azim_angle=-75)

        img_list = [image, skeleton_overlay, skeleton_3d_cam_view, skeleton_3d_another_view]
