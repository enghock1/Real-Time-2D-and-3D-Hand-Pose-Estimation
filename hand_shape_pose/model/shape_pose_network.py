...
class ShapePoseNetwork(nn.Module):
    ...
    def forward(self, images, cam_param, pose_scale, pose_root=None, bbox=None):
        ...
        #print(torch.sum(encoding[0]))
        #print(torch.sum(encoding[1]))
        ...
        if bbox is None:
            if pose_root is None:
                est_mesh_cam_xyz = uvd2xyz_freihand_test(est_mesh_uvd, cam_param, pose_scale)  # B x V x 3
            else:
                est_mesh_cam_xyz = uvd2xyz_freihand_train(est_mesh_uvd, cam_param, root_depth, pose_scale)  # B x V x 3
        else:
        ...
        if bbox is None:
            if pose_root is None:
                est_pose_cam_xyz = uvd2xyz_freihand_test(est_pose_uvd, root_uv, cam_param, pose_scale)  # B x (K-1) x 3
            else:
