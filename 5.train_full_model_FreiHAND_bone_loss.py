...
def main():
    ...
    for epoch in range(num_epoch):
        ...
        for i, batch in enumerate(data_loader_val):
            ...
            loss = 0.1*hm_loss(est_hm_list, gt_heatmap) + pose_loss(est_pose_cam_xyz, gt_pose_cam_xyz) + 0.01*len_loss + 0.1*dir_loss

            # backpropagation
            loss.backward()
            optimizer.step()
            total_loss_train += loss.item()
            tot_loss_len += len_loss.item()*0.01
            tot_loss_dir += dir_loss.item()*0.1
